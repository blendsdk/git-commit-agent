...
const git_status_tool = tool(
    async (): Promise<string> => {
        ...
        try {
            ...
            const pagerConfig = await executeGitCommand(
                ["config", "--global", "pager.diff", "false"],
                { required: false }
            );
            ...
            const diff = await executeGitCommand(
                ["diff", "--name-only"],
            ...
            const cached = await executeGitCommand(
    ...
    {
        name: "git_status",
        description: "Get the current status of the git repository including branch, changed files, and staged files. Returns detailed status with error handling."
    }
);
...
const git_diff_tool = tool(
    ...
    {
        name: "git_diff",
        description: "Get the detailed diff of changes in the git repository. Optionally specify files to diff. Returns both unstaged and staged changes with statistics.",
        schema: z.object({
            files: z.array(z.string()).optional().describe("Optional array of file paths to get diff for specific files")
        })
    }
);
...
const git_add_tool = tool(
    async ({ files, all }: { files?: string[]; all?: boolean }): Promise<string> => {
        ...
        try {
            ...
            const statusCheck = await executeGitCommand(
                ["status", "--porcelain"],
            ...
            const staged = await executeGitCommand(
                ["diff", "--name-only", "--cached"],
                { required: false }
            );
    ...
    {
        name: "git_add",
        description: "Stage changes in the git repository. Can stage all changes or specific files. Validates files exist before staging.",
        schema: z.object({
...
const git_commit_tool = tool(
    async ({ commit_message, validate }: { commit_message: string; validate?: boolean }): Promise<string> => {
        ...
        try {
            ...
            const stagedCheck = await executeGitCommand(
    ...
    {
        name: "git_commit",
        description: "Commit staged changes with a commit message. Validates conventional commit format and ensures staged changes exist. Returns commit hash and details.",
        schema: z.object({
...
Use the:
    - git_status tool to get the current status of the repository and the list of changed files.
    - git_add tool to stage all changes.
    - git_commit tool to commit the staged changes with the generated commit message.
    - git_diff tool to get detailed differences if needed.
